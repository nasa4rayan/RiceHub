...
# Theme: Tokyo Night Openbox

set -e

THEME_SLUG="tokyo-night-openbox"
WM_DE="openbox"
...
copy_dotfiles() {
    ...
    # Copy Openbox config
    if [ -d "dotfiles/openbox" ]; then
        mkdir -p "$HOME/.config/openbox"
        cp -r dotfiles/openbox/* "$HOME/.config/openbox/"
        log_info "Copied Openbox config"
    fi
    ...
    if [ -d "dotfiles/polybar" ]; then
        ...
        log_info "Copied Polybar config"
    fi
...
main() {
    ...
    case "$DETECTED_DISTRO" in
        arch)
            install_packages "$DETECTED_DISTRO" picom rofi polybar kitty feh ttf-jetbrains-mono-nerd
            ;;
        debian)
            install_packages "$DETECTED_DISTRO" picom rofi polybar kitty feh fonts-jetbrains-mono
            ;;
        fedora)
            install_packages "$DETECTED_DISTRO" picom rofi polybar kitty feh jetbrains-mono-fonts
            ;;
    esac
